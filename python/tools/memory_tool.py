...
from OpenQuantumAI import OpenQuantumAI
...
def search(agent:OpenQuantumAI, query:str, count:int=5, threshold:float=0.1):
    initialize(agent)
...
def forget(agent:OpenQuantumAI, query:str):
    initialize(agent)
    deleted = db.delete_documents_by_query(query) # type: ignore
    return files.read_file("./prompts/fw.memories_deleted.md", memory_count=deleted)

def initialize(agent:OpenQuantumAI):
    global db
    if not db:
